...
- **After Wave 3 / before `$gsd-verify-work`:** Prove the race selector resolves exactly 28 live top-level tests and the native selector resolves exactly 4, then run the full short suite, the targeted race selector above, `go test -run '^$' ./...`, exported-surface `go doc` checks, `go test -short ./ort -run '^TestBootstrapCreatedFilePermissions$'`, configured native non-race tests, `go vet -unsafeptr=false ./ort/...`, and `make precommit-lint-new`
...
| 02-08-02 | 02-08 | 3 | API-02, API-03 | T-02-02 / T-02-03 / T-02-07 / T-02-08 / T-02-09 / T-02-10 / T-02-SC | CI keeps fake ownership/concurrency proofs under race and real ABI/model proofs in the configured non-race lane; each selector proves 28/4 live top-level tests before execution, and exported APIs plus unchanged action references are checked while comprehensive suites run at wave/phase scope | CI/liveness + wave/phase gate | `go test -list '<race-selector>' ./ort` resolves 28 tests, `go test -list '<native-selector>' ./ort` resolves 4 tests, and `test -z "$(git diff HEAD --unified=0 -- .github/workflows/ci.yml | sed -n '/^[+-][[:space:]]*uses:/p')"` | ⚠️ extend existing workflow | ⬜ pending |
...
- [ ] Flow-test additions (02-04-02/02-05-01/02-06-01/02-06-02/02-07-01/02-07-03) in `environment_test.go`, `memory_test.go`, `tensor_test.go`, `session_test.go`, and `bootstrap_test.go` — `errors.Is`/`errors.As`, preserved causes, and approved diagnostic call sites for API-02
